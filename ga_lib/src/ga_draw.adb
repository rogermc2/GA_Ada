...
package body GA_Draw is
   ...
   procedure Draw_Circle (Palet_Type          : Palet.Colour_Palet;
                          Scaled_Model_Matrix : GL.Types.Singles.Matrix4;
      ...
      procedure Draw_Hooks is
         ...
         Hooks          : constant Vector2_Array (1 .. 2) :=
                            ((1.0, 0.0),
                             (1.0, -1.5));
         Vertex_Buffer  : GL.Objects.Buffers.Buffer;
         Model          : Matrix4 := Scaled_Model_Matrix;
         Hook_Rotation  : constant Matrix4 := Maths.Rotation_Matrix
      ...
      begin
         ...
         for index in Int range 1 ..6 loop
            GL.Objects.Vertex_Arrays.Draw_Arrays (Mode  => Lines,
                                                  First => 0,
                                                  Count => 2);
            Model := Hook_Rotation * Model;
            Shader_Manager.Set_Model_Matrix (Model);
         end loop;
         Shader_Manager.Set_Model_Matrix (Scaled_Model_Matrix);
      end Draw_Hooks;
