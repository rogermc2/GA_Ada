...
--     end Pick_Matrix;
   ...
   function To_GL (V3 : Multivectors.Multivector) return GL.Types.Doubles.Vector3 is
      ...
      use Blade_Types;
      use GA_Maths;
      Blades  : constant Multivectors.Blade_List :=
                  Multivectors.Get_Blade_List (V3);
   ...
   procedure Viewport_Coordinates (Pt_World          : GA_Maths.Array_3D;
      ...
      Window_Width  : Size;
      Window_Height : Size;
      PT1           : constant Vector4 := (Single (Pt_World (1)),
                                           Single (Pt_World (2)),
                                           Single (Pt_World (3)), 1.0);
      PT2           : constant Vector4 := Projection_Matrix *
                        Model_View_Matrix * PT1;
   begin
      --   PT1 := Projection_Matrix * PT2;
      GL.Window.Get_Viewport (VP_X, VP_Y, Window_Width, Window_Height);
      Coords (X) := Single (VP_X) + (1.0 + PT2 (X) / PT2 (W)) *
        Single (Window_Width) / 2.0;
      Coords (Y) := Single (VP_Y) + (1.0 + PT2 (Y) / PT2 (W)) *
        Single (Window_Height) / 2.0;
