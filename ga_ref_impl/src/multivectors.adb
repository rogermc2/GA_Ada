...
package body Multivectors is
         ...
         begin
            ...
            if Cond < 1.0 / Float'Model_Epsilon then
               ...
               raise MV_Exception with
                 "Multivectors.General_Inverse cannot invert matrix as it is singular.";
            end if;
         end; --  declare block
      end if;
      Simplify (Result);
   ...
   --  -------------------------------------------------------------------------
   --  Contraction functions based on Stephane Breuils'
   --  Algorithmic structure for geometric algebra operators and
   --  application to quadric surfaces. Operator Algebras [math.OA].
