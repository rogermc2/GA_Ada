...
package body C3GA_Utilities is
   ...
   function C3GA_Point (V : Vector_E3GA) return C3GA.Normalized_Point is
      Norm   : constant C3GA.Scalar := C3GA.Norm_E2 (V);
      Offset : constant float := 0.5 * C3GA.Coord (Norm) * C3GA.NI;
      thePoint : C3GA.Normalized_Point;
   begin
      thePoint := C3GA.Set_Normalized_Point (E3GA.Get_Coord_1 (V) + Offset,
                       E3GA.Get_Coord_2 (V) + Offset,
                       E3GA.Get_Coord_3 (V) + Offset, C3GA.NI);
      return thePoint;
   end C3GA_Point;
